...
Job.prototype.finish = function() {
    this.handled = true
    this.state = 'finished'
    return this.db.none('UPDATE "JobQueue" SET "state"=${state}, "lastRun"=NOW() WHERE "id"=${id}', this)
...
Job.prototype.destroy = function() {
    ...
    return this.db.none('DELETE FROM "JobQueue" WHERE "id"=${id}', this)
...
Job.prototype.reschedule = function(date) {
    ...
    return this.db.none('UPDATE "JobQueue" SET "state"=${state}, "scheduledFor"=${scheduledFor}, "lastRun"=NOW() WHERE "id"=${id}', this)
...
Job.prototype.fail = function (error, rescheduleFor) {
    ...
    return self.db.tx(function*(t) {
        ...
        let JobQueue = yield t.one('SELECT * FROM "JobQueue" WHERE "id"=${id}', self);
        if (JobQueue.failedAttempts >= JobQueue.maxAttempts) {
            // complete failure, ensure the job isn't run again
            self.state = 'failed';
            return yield t.none('UPDATE "JobQueue" SET "state"=${state} WHERE "id"=${id}', self);
        }
        ...
        self.state = 'waiting';
        return yield t.none('UPDATE "JobQueue" SET "state"=${state}, "scheduledFor"=${scheduledFor} WHERE "id"=${id}', self);
    })
