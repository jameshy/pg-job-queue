...
function addJob(job) {
    if (!job.type) {
        throw new Error('no job.type not specified')
    }
    
    return db.none('INSERT INTO "JobQueue" ("type", "data", "scheduledFor", "maxAttempts", "createdAt") VALUES (${type}, ${data}, ${scheduledFor}, ${maxAttempts}, NOW() )', {
        type: job.type,
        data: job.data || {},
        scheduledFor: pgp.as.date(job.scheduledFor || new Date()),
